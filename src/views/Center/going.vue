<template>
  <div>
    <van-nav-bar title="学习中课程" left-text="返回" left-arrow @click-left="onClickLeft" />
    <div class="order" v-for="(item, index) in dataLists1" :key="'suc' + index">
      ...
      <van-card class="details" >
        <template #desc>
          <div v-show='item.id == id2[index]'>课程剩余节数：{{number[index]}}节</div>
        ...
        <template #footer>
          ...
          >转校申请</van-button>
          <van-button class="xfBut" square type="primary" color="#3279FD" @click="xfBut(item)">立即续费</van-button>
        </template>
      </van-card>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    xfBut: function(val) {
      // console.log(val)
      // window.localStorage.setItem('BuyData',JSON.stringify(val))
